...
class Validation
{
	 ...
	 */
	public static function valid(array $data, array $validators)
	{
		$gump = new Validation();
	...
	public static function add_validator($rule, $callback)
	{
		...
		if(method_exists(__CLASS__, $method) || isset(self::$validation_methods[$rule])) {
			throw new vuHelperException("Validator rule '$rule' already exists.");
		}
	...
	public static function add_filter($rule, $callback)
	{
		...
		if(method_exists(__CLASS__, $method) || isset(self::$filter_methods[$rule])) {
			throw new vuHelperException("Filter rule '$rule' already exists.");
		}
		...
		foreach($ruleset as $field => $rules)
		{
	        ...
	        {			
				foreach($rules as $rule)
				{
					...
					{
						throw new vuHelperException("Validator method '$method' does not exist.");
					}
				}
			}
		}
	...
	public function filter(array $input, array $filterset)
	{
		foreach($filterset as $field => $filters)
		{
			...
			foreach($filters as $filter)
			{
				...
				{
					throw new vuHelperException("Filter method '$filter' does not exist.");
				}
			}
		}
		return $input;
